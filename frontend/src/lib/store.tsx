        ...
        if (detailData?.statusHistory && Array.isArray(detailData.statusHistory)) {
          setStatusHistory((prev) => ({
            ...
            [issueId]: res.data!.statusHistory!.map((h: Record<string, unknown>) => ({
              ...
              changedByName: (h.changedByName as string) || (h.toStatus === 'REPORTED' && (detailData.reporter as any)?.name) || 'Campus Member',
              changedByRole: (((h.changedByRole as string) || (h.toStatus === 'REPORTED' && (detailData.reporter as any)?.role) || 'OFFICIAL').toUpperCase()) as UserRole,
